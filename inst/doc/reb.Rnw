...
To make use of these tools you will need to have Bioconductor installed along with the \Rpackage{ideogram} 
package. These must be installed in your version of R and when you start R you must load them with the
\Rfunction{library} command. The \Rpackage{ideogram} package duplicates some of the functionality found in 
...
for(i in c("2","5","11","6")) 
ideogram(data,vai.chr,method="i",col=.rwb,dlim=c(-4,4),chr=i)
par(op)
@
\end{center}
...
\begin{center}
<<fig=TRUE, echo=TRUE>>=
ideogram(mcr.cset@exprs[,complex.ix],vai.chr,
